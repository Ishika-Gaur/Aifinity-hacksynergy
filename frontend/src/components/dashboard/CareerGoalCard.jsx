...
export default function CareerGoalCard({ careerGoal, onUpdateGoal }) {
  ...
  const modalContent = isOpen ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1B332C]/70 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg rounded-md bg-[#FBF8F0] border-2 border-[#C4952A] p-6 shadow-2xl flex flex-col gap-5 animate-in fade-in zoom-in-95 duration-200">
        {/* Modal Header */}
        <div className="flex items-center justify-between border-b border-[#2E4F42]/15 pb-3">
          <div>
            <h3 className="font-['Kalam'] text-2xl font-bold text-[#1B332C]">
              Update Career Goal
            </h3>
            <p className="text-xs text-[#5B6B5F] font-sans">
          ...
          <button
            ...
            className="rounded h-8 w-8 inline-flex items-center justify-center text-[#5B6B5F] hover:bg-[#EDE6D3] hover:text-[#1B332C] transition-colors font-bold text-lg cursor-pointer"
          >
            ✕
          </button>
        </div>
        ...
        <div className="flex flex-col gap-2">
          <label className="font-['Space_Mono'] text-xs font-bold uppercase tracking-wider text-[#1B332C]">
            ...
            {CAREER_PRESETS.map((preset, idx) => {
              ...
              return (
                <button
                  ...
                  className={`rounded px-3 py-1.5 text-xs font-sans font-medium transition-all text-left border cursor-pointer ${
                    isSelected
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              required
              className="w-full rounded border border-[#2E4F42]/20 bg-white px-3.5 py-2 text-sm text-[#1B332C] focus:border-[#C4952A] focus:outline-none focus:ring-1 focus:ring-[#C4952A]"
            />
          </div>
  ...
  return (
    <div className="relative rounded-md bg-[#FBF8F0] p-6 border border-[#2E4F42]/12 shadow-[var(--shadow-card)] hover:shadow-[var(--shadow-card-hover)] hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between gap-4 group">
      {/* Toast Notification on Success */}
      {showSuccessToast && (
        <div className="absolute -top-3 inset-x-4 z-20 rounded bg-[#1B332C] px-3.5 py-2 text-xs text-[#E8C547] border border-[#E8C547]/50 shadow-md flex items-center justify-between font-['Space_Mono']">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          <h4 className="font-['Kalam'] text-2xl font-bold text-[#1B332C]">
            {currentGoal.role}
          </h4>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {currentGoal.tags.map((tag, idx) => (
            <span
              key={idx}
              className="rounded bg-[#EDE6D3] px-2.5 py-0.5 font-['Space_Mono'] text-[11px] font-semibold text-[#1B332C] hover:bg-[#1B332C] hover:text-[#E8C547] transition-colors cursor-pointer"
            >
              {tag}
            </span>
          ))}
